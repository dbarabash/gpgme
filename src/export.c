...
export_start (gpgme_ctx_t ctx, int synchronous, const char *pattern,
	      gpgme_export_mode_t mode, gpgme_data_t keydata, int secret)
{
  gpgme_error_t err;
  ...
  return _gpgme_engine_op_export (ctx->engine, pattern, mode, keydata,
				  ctx->use_armor, secret, ctx->keyserver);
}
		       ...
		       gpgme_export_mode_t mode, gpgme_data_t keydata, int secret)
{
  ...
  err = export_start (ctx, 0, pattern, mode, keydata, secret);
		 ...
		 gpgme_export_mode_t mode, gpgme_data_t keydata, int secret)
{
  ...
  err = export_start (ctx, 1, pattern, mode, keydata, secret);
  if (!err)
